...
// a structure that represents a basic variable containing its label (ranges from 0 to m + n - 1) and its current value
struct variable {
    size_t label;
    double value;
};
...
// print out the final values of decision variables and slack variables when the optimal solution is reached
...
int main(int argc, const char * argv[]) {
    ...
    // Proceed with the Simplex Method
    ...
    while (true) {
        ...
        // choose an entering column using the condition Cn > ya, where "a" is a column of An

        // a vector to keep track of the variables whose coefficients in the objective function in this iteration are positive
        ...
        // At this point we have a pair of entering and leaving variable so that the entering variable is positive and the diagonal entry in the eta column of the eta matrix is fairly far from zero
